...
const handler: APIGatewayProxyHandler = async event => {
  ...
  if (!body || !body.sessionId || !body.userId || !body.character) {
    return {
      statusCode: 400,
  ...
  if (body.character.userId !== body.userId) {
    return {
      statusCode: 403,
      body: JSON.stringify({ detail: 'Cannot create characters for other users.' })
    };
  }
  ...
  const putCommand = new PutCommand({
    ...
    Item: {
      ...
      itemId: `character#${body.character.id}`,
      userId: body.userId,
